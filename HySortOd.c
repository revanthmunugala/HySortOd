...
// run example with 10 lines 3 dimensions 4 parititions per dim(Bin)
...
int isPresent(struct Hypercube *hypercube, struct hypercube *curHypercube,
              int DIM);
void appendHypercube(struct Hypercube **hypercube,
                     struct hypercube *curHypercube, int DIM);
void createHypercube(struct Hypercube **hypercube, double **dataset, int N,
                     int DIM, int BIN);
int compfn(const void *a, const void *b);
void sort(struct Hypercube **hypercube);
struct treeNode *appendNode(struct treeNode *root, int begin, int end,
                            int coordinate);
void buildTree(struct Hypercube *hypercube, struct treeNode *root, int DIM,
               int curDim, int MINSPLIT);
int neighborhood_density(struct Hypercube *hypercube, struct treeNode *root,
                         int hypercubeIndex, int curDim);
void outlierScore(struct Hypercube *hypercube, struct treeNode *root,
                  int *neighborDensity, int *maxDensity, int N, int DIM);
...
int main(int argc, char **argv) {
  ...
  if (argc != 7) {
    fprintf(stderr,
            ...
            "Normalize (0 or 1)dataset filename. Your input: %s\n",
            argv[0]);
    return 0;
  }
...
void appendHypercube(struct Hypercube **hypercube,
  ...
  (*hypercube)->grid = realloc(
      (*hypercube)->grid, sizeof(struct hypercube) * ((*hypercube)->count + 1));
  (*curHypercube).instancesCount = 1;
...
void createHypercube(struct Hypercube **hypercube, double **dataset, int N,
  ...
  for (int i = 0; i < N; i++) {
    ...
    if (index == -1) {
      ...
      (*hypercube)->grid[index].instancesCount++;
    }
  }
  return;
}
...
void sort(struct Hypercube **hypercube) {
  qsort((*hypercube)->grid, (*hypercube)->count, sizeof(struct hypercube),
        compfn);
  return;
}
  ...
  if (root->nextLevel == NULL) {
    while (root != NULL) {
      for (int i = root->curNode.startIndex; i <= root->curNode.endIndex; i++) {
        int curDiff = abs(hypercube->grid[hypercubeIndex].dimensions[curDim] -
                          hypercube->grid[i].dimensions[curDim]);
        if (curDiff <= 1) {
          curDensity += hypercube->grid[i].instancesCount;
        }
      }
      root = root->nextNode;
    }
    ...
    while (root != NULL) {
      int curDiff = abs(root->curNode.coordinate -
                        hypercube->grid[hypercubeIndex].dimensions[curDim]);
      if (curDiff <= 1) {
        curDensity += neighborhood_density(hypercube, root->nextLevel,
                                           hypercubeIndex, curDim + 1);
      }
      root = root->nextNode;
    }
  }
...
// Function to find max density and calculate neighborhood density of each
// hypercube
void outlierScore(struct Hypercube *hypercube, struct treeNode *root,
                  int *neighborDensity, int *maxDensity, int N, int DIM) {
  for (int i = 0; i < hypercube->count; i++) {
...
void printOutlierScore(int *density, int maxDensity, int count) {
  for (int i = 0; i < count; i++) {
    printf("Hypercube index:%d Outlier score: %lf\n", i,
           (float)(maxDensity - density[i]) / (float)maxDensity);
  }
